...
export class WorkflowEditorClientRPC extends BaseClientRPCRequester {
  private readonly callerLogger = new Logger(WorkflowEditorClientRPC.name)
  ...
  getRequestSchema<K extends keyof ClientRPCMethods>(
    ...
    this.callerLogger.debug(`Getting request schema for method: ${method}`)
  ...
  getResponseSchema<K extends keyof ClientRPCMethods>(
    ...
    this.callerLogger.debug(`Getting response schema for method: ${method}`)
