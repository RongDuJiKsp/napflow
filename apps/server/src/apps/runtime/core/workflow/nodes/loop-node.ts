import { ComponentNodesEnum, ZodCheckComponentNodeMeta } from '@shared/common/workflow/component-node'
...
export class LoopNode extends CommNode<LoopDataCtx> {
  ...
  onThread(
    ...
    if (_nkv['loop.index'] >= this.data.maxCount)
      return

    // 获取子图的头节点
    const headNodes = thread.plugin.commNodes.filter(node => node.parentId === this.id && node.data.type === ComponentNodesEnum.LoopStart)
    ...
    for (const node of runableNodes)
  ...
  getRunner(thread: WorkflowThread): GraphRunner {
    const subNodes = thread.plugin.commNodes.filter(node => node.parentId === this.id)
    const subNodeIdSet = new Set(subNodes.map(node => node.id))
    const subEdges = thread.plugin.commEdges.filter(edge => subNodeIdSet.has(edge.source) || subNodeIdSet.has(edge.target))
