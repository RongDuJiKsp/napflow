...
export class BotBridgeForBotService {
  ...
  async getBindingsInfo(botId: string) {
    ...
    const getAppString = (
      app: Pick<WorkflowAppDataEntity, 'ofAppId' | 'version'>,
    ) => `[appId=${app.ofAppId},version=${app.version}]`
    const appMap = Object.fromEntries(
      bindingApp.map(app => [getAppString(app), app]),
    )
    return botRecord.commonAdapterConfig.bindingWorkflowApp?.map(
      ({ appId, version, bindingId }) => ({
        appId,
        version,
        bindingId,
        appPublish: appMap[getAppString({ ofAppId: appId, version })],
      }),
    )
  }
}
