...
export class NapcatWsSdk extends NCWebsocket {
  ...
  async parseChain(
    ...
    return (
      await Promise.all(chain.map(seg => this.parseSegment(seg, { deep })))
    ).join(' ')
  }
  ...
  async parseSegment(
    ...
    switch (msg.type) {
      case 'text':
        return this.parseText(msg)
      case 'at':
        return this.parseAt(msg)
      case 'image':
        return this.parseImage(msg)
      case 'file':
        return this.parseFile(msg)
      case 'video':
        return this.parseVideo(msg)
      case 'record':
        return this.parseRecord(msg)
      case 'poke':
        return this.parsePoke(msg)
      case 'dice':
        return this.parseDice(msg)
      case 'rps':
        return this.parseRps(msg)
      case 'face':
        return this.parseFace(msg)
      case 'reply':
        return this.parseReply(msg, { deep })
      case 'forward':
        return this.parseForward(msg)
      case 'json':
        return this.parseJson(msg)
      case 'markdown':
        return this.parseMarkdown(msg)
      default:
        return ''
    }
  }
  ...
  async parseAt(msg: Receive['at']): Promise<string> {
  ...
  async parseReply(
    ...
    if (!deep) return `[回复 @${origin.sender.nickname}]`
   ...
   * forward
   * ====================== */
  async parseForward(_: Receive['forward']): Promise<string> {
    return '[转发消息]'
  }
