...
export class NcReplyNode extends ReplyNode {
  ...
  async compileAndReply(
    ...
    if (this.data.replyTarget === ReplyTarget.Group && this.data.groupId) {
      ...
      this.data.replyTarget === ReplyTarget.triggerSource
      ...
      if (sourceKv['trigger.uid']) {
        await this.replyToUser(
          thread,
          String(sourceKv['trigger.uid']),
          content,
        )
      }
      else if (sourceKv['trigger.gid']) {
        await this.replyToGroup(
          thread,
          String(sourceKv['trigger.gid']),
          content,
        )
      }
      else {
