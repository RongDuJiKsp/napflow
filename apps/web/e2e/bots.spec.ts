...
test.describe('机器人列表功能', () => {
  ...
  test('成功创建使用NapcatWs的适配器', async ({
    ...
    await aiInput(
      `由 midscene 自动创建的 bot，创建时间为${creationTime}`,
      '机器人描述输入框',
    )
    ...
    await aiInput(
      E2eEnvs.E2E_NAPFLOW_WS_MAX_RETRIES.toString(),
      '最大重连次数输入框',
    )
    await aiInput(
      E2eEnvs.E2E_NAPFLOW_WS_RECONNECT_INTERVAL.toString(),
      '重连间隔（ms）输入框',
    )
    await aiInput(
      E2eEnvs.E2E_NAPFLOW_WS_HEARTBEAT_INTERVAL.toString(),
      '心跳间隔（ms）输入框',
    )
    ...
    await expect(
      page.getByRole('heading', { name: /^mids-bot-/ }).first(),
    ).toBeVisible()
    await aiAssert(
      `页面机器人列表包含新创建的机器人卡片，描述中包含的创建时间为${creationTime}，新创建的机器人卡片展示适配器为 Napcat Ws Client`,
    )
  })

  test('Napcat Ws 配置 badcase: WebSocket 地址非法', async ({
    page,
    aiInput,
    aiTap,
  }) => {
  ...
  test('Napcat Ws 配置 badcase: 重连间隔小于 1000ms', async ({
    page,
    aiInput,
    aiTap,
  }) => {
  ...
  test('Napcat Ws 配置 badcase: WebSocket 地址为空', async ({
    page,
    aiInput,
    aiTap,
  }) => {
