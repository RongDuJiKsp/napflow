...
import { $createEnvVarNode, $isEnvVarNode, LexEnvVarNode } from '../lex-nodes/env-var-node'
...
const EnvVarSyncPlugin = ({ envVars}: { envVars: VarCtx[] }) => {
  ...
  useEffect(() => {
    editor.update(() => {
      for(const envVarNode of $dfs().filter(dfs => $isEnvVarNode(dfs.node)).map(dfs => dfs.node as LexEnvVarNode))
