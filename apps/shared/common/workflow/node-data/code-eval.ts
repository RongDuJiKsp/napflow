...
export const JsValueTransform = defineTypedRecord<
  Record<VarTypes, (from: unknown) => unknown>
>({
  ...
  [VarTypes.StringArray]: (from) => {
    if (typeof from === 'string') {
      ...
      if (Array.isArray(tryParsed)) return tryParsed.map(String)
    }

    if (Array.isArray(from)) return from.map(String)
  ...
  [VarTypes.NumberArray]: (from) => {
    if (typeof from === 'string') {
      const tryParsed = tryParseJson(from)
      if (Array.isArray(tryParsed)) return tryParsed.map(Number)
    }

    if (Array.isArray(from)) return from.map(Number)

    return [Number(from)]
  },
})

export const CodeEvalDataSchema = z
  .object({
