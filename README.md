...
### 🧠 Agent 能力
- **智能对话支持**：在工作流编辑场景中提供 AI 辅助对话能力
- **会话连续体验**：支持历史会话恢复，减少中断后的重复沟通成本
- **上下文记忆回显**：恢复会话时可继续查看既有对话内容
- **模型配置管理**：支持模型配置的新增、更新、删除和列表查看（密钥脱敏）
- **多应用会话管理**：支持按应用维度管理与恢复会话
